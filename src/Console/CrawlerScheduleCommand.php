...
class CrawlerScheduleCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $data = collect();
        $page_from = (int)Option::get('crawler_schedule_page_from', 1);
        $page_to = (int)Option::get('crawler_schedule_page_to', 2);
        
        // Xác định thứ tự đúng của trang
        $start = max($page_from, $page_to);
        $end = min($page_from, $page_to);
        
        $this->logger->notice(sprintf("Crawler Page (FROM: %d | TO: %d)",  $start, $end));
        
        // Lấy danh sách phim từ page cao đến page thấp
        for ($i = $start; $i >= $end; $i--) {
            ...
            if ($response['status'] && count($response['items'])) {
                // Đảo ngược thứ tự item trong mỗi trang
                $items = array_reverse($response['items']);
                $data->push(...$items);
            }
        }
        $movies = $data->shuffle();
